...
def _strip_unknown_kwargs(kwargs: dict) -> dict:
    """Remove fragile/unknown params that often break across model/SDK updates."""
...
def classification_CLIP_0_shot(
    text_path,
    img_dir=None,
    mode=None,                      # "text" | "image" | "both"
    prompt=None,                    # list[str] for CLIP labels
    text_column=["headline"],
    predict_column="label",
    # --- new ---
    text_backend="clip",            # "clip" | "llama"
    llama_model="TinyLlama/TinyLlama-1.1B-Chat-v1.0",
    llama_max_new_tokens=8):
    """
    Zero-shot classification.
    If mode == 'text':
        - text_backend='clip'  -> use CLIP's text encoder vs prompt labels
        - text_backend='llama' -> use a tiny LLaMA to pick a label via prompting
    If mode includes image -> behaves like before (CLIP image/text encoders).
    """
    if mode not in ["text", "image", "both"]:
        raise ValueError("mode must be 'text', 'image', or 'both'")
    if prompt is None:
        # default label list; replace with your own
        prompt = ["politics","sports","business","tech","entertainment","health","science","world"]
    ...
    # -------- helper: get sample text --------
    def _row_text(row):
        return " ".join(str(row[c]).strip() for c in text_column
                        if c in row and pd.notna(row[c]))

    # -------- branch A: CLIP backend for text/both (existing behavior) --------
    if text_backend.lower() == "clip" or mode in ("image", "both"):
        model     = CLIPModel.from_pretrained(base_model).to(device)
        processor = CLIPProcessor.from_pretrained(base_model)
        # encode label prompts once
        with torch.no_grad():
            t_inputs = processor(text=prompt, return_tensors="pt", padding=True, truncation=True).to(device)
            prompt_features = F.normalize(model.get_text_features(**t_inputs), p=2, dim=1)

        predictions = []
        for _, row in tqdm(df.iterrows(), total=len(df), desc="Predicting"):
            sample_text = _row_text(row) if use_text else ""
            if use_image:
                img_path = os.path.join(img_dir, f"{row.get('image_id', row.name)}.jpg")
            ...
            with torch.no_grad():
                if use_text and use_image:
                    inputs  = processor(text=sample_text, images=image, return_tensors="pt",
                    ...
                    feat    = F.normalize((text_f + img_f) / 2, p=2, dim=1)
                elif use_text:
                    inputs  = processor(text=sample_text, return_tensors="pt",
                                        padding=True, truncation=True).to(device)
                    feat    = F.normalize(model.get_text_features(**inputs), p=2, dim=1)
                else:
                    inputs  = processor(images=image, return_tensors="pt").to(device)
                    feat    = F.normalize(model.get_image_features(inputs.pixel_values), p=2, dim=1)

                sim      = feat @ prompt_features.t()
                pred_idx = sim.argmax().item()
                # label ids start at 1 for compatibility with your pipeline
                predictions.append(pred_idx + 1)

        df[predict_column] = predictions
        return df

    # -------- branch B: LLaMA backend for TEXT MODE --------
    if mode != "text":
        raise ValueError("text_backend='llama' is only supported in mode='text'")

    # load tiny llama
    torch_dtype = torch.float16 if device.type == "cuda" else torch.float32
    tok  = AutoTokenizer.from_pretrained(llama_model)
    lmr  = AutoModelForCausalLM.from_pretrained(llama_model, torch_dtype=torch_dtype, device_map="auto" if torch.cuda.is_available() else None)
    if tok.pad_token is None:
        tok.pad_token = tok.eos_token

    # Build a strict instruction once
    allowed_ids = ", ".join(str(i) for i in range(1, len(prompt)+1))
    sys_inst = (
        "You are a strict news classifier.\n"
        f"Valid class ids: [{allowed_ids}].\n"
        "Return EXACTLY ONE id (just the number). No words, no punctuation."
    )

    def _llama_classify(text: str) -> int:
        msg = f"{sys_inst}\n\nCategories:\n" + "\n".join([f"{i+1}. {name}" for i, name in enumerate(prompt)]) + \
              f"\n\nText:\n{text}\n\nAnswer:"
        inputs = tok(msg, return_tensors="pt", truncation=True, max_length=2048).to(lmr.device)
        out = lmr.generate(**inputs, max_new_tokens=llama_max_new_tokens, do_sample=False)
        ans = tok.decode(out[0][inputs['input_ids'].shape[1]:], skip_special_tokens=True).strip()
        # extract first integer
        m = re.search(r"\b(\d{1,3})\b", ans)
        if not m:
            return 99
        try:
    ...
    predictions = []
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Predicting (LLaMA)"):
        txt = _row_text(row)
        pred = _llama_classify(txt)
...
class GPTClassifier:
    ...
    def _wrap_responses_output(resp):
        """Normalize Responses API output to look like Chat Completions (resp.choices[0].message.content)."""
        try:
    ...
    # --- Core API call -------------------------------------------
    def fetch(self, messages, model, reasoning_effort, n):
        """
        Resilient call:
          - Uses Responses API for gpt-5* models (accepts reasoning.effort)
          - Uses Chat Completions otherwise
          - On model/param failure: tries fallbacks, strips fragile kwargs, and finally shows FRIENDLY_MODEL_MESSAGE
        """
        # Prepare the candidate list (requested model → fallbacks)
        candidates = [model] + _MODEL_FALLBACKS.get(model, [])

        last_err = None

        # First attempt keeps your intent (including reasoning for gpt-5*)
        for pass_id in (0, 1):  # pass 0: keep kwargs; pass 1: strip to safe defaults
            for cand in candidates:
                try:
                    if cand.startswith("gpt-5"):
                        # Responses API (supports reasoning.effort)
                        ...
                        if pass_id == 1:
                            # strip to conservative defaults
                    ...
                    if pass_id == 1:
                        # add safe defaults, but no exotic kwargs
                    ...
                    if _is_model_or_param_error(e):
                        # Try next candidate or next pass
                        continue
                    # For non-model errors (network/auth), stop early
                    break

            # end for cand
        # end for pass_id

        # If we got here, everything failed → print friendly guidance and raise
...
def classification_GPT(
    ...
    num_votes: int = 1,
    batch_size: int = 5,
    wait_time: float = 1.2) -> pd.DataFrame:

    print_env_info()  # helps reviewers reproduce
    ...
    # default: non-reasoning (traditional mode)
    ...
    # minimal → non-reasoning path (no explicit reasoning call)
    ...
    for start in range(0, n, max(1, batch_size)):
        ...
        for col, lab, pr in tasks:
            ...
            if first:
                sub, _ = clf.classify_df(sub, task, return_sample_response=True)
                first = False
            else:
                sub = clf.classify_df(sub, task)

        outputs.append(sub)

        # gentle pause between chunks to stay under TPM
        time.sleep(wait_time)

    # merge chunk results back
